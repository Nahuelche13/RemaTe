...
public class EmpleadoDA {
    public static async Task<Errors> Create(EmpleadoVO empleado) {
        ...
        cmd.CommandText = "INSERT INTO empleado (ci) VALUES ($ci);";
    ...
    public static async IAsyncEnumerable<EmpleadoVO> ReadAllWithUsuarioWhere(UsuarioVO filter) {
        var command = DBC.I.DbConn.CreateCommand();
        var sb = new StringBuilder();
        sb.AppendLine(@"
            SELECT e.ci, u.nombre, u.email, u.telefono, u.departamento, u.localidad, u.calle, u.puerta, u.permisos, u.is_active
            FROM empleado e
            JOIN usuario u ON u.id == e.ci
        ");
